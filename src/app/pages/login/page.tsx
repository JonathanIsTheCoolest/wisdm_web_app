...
const OnboardingPage = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      const result = await authFunctionWrapper(getUser());
      if (result?.redirect) {
  ...
  return (
    <div className={styles.startScreenWrapper}>
      ...
      />
      <h1>WISDM</h1>
      <Link href="/pages/login/signup" className={styles.signupButton}>
        Sign Up
      </Link>
      <Link href="/pages/login/signin" className={styles.loginButton}>
        Log In
      </Link>
    </div>
  );
