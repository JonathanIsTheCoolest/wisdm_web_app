...
const OnboardingPage = ({ signup, login }) => (
    <div className={styles.signupWrapper}>
        ...
        <button onClick={login} className={styles.loginButton}>Log In</button>
    </div>
);

const SignUpPage = ({ onboarding, personalInfo }) => (
    <div className={styles.signupPage}>
        <button onClick={onboarding} className={styles.backButton}>←</button>
        <h2>Let's Get Started</h2>
        <label>
            Full Name
            <input type="text" placeholder="Insert Name" className={styles.inputField} />
        </label>
        <label>
            Email
        ...
        <button onClick={personalInfo} className={styles.nextButton}>Next</button>
    </div>
);
...
const LoginPage = ({ onboarding, forgotPassword }) => (
    <div className={styles.loginPage}>
        ...
        <a href="#" onClick={forgotPassword} className={styles.forgotPassword}>Forgot Password?</a>
        <button onClick={onboarding} className={styles.nextButton}>Log in</button>
    </div>
);

const ForgotPasswordPage = ({ login }) => (
    <div className={styles.forgotPasswordPage}>
        <button onClick={login} className={styles.backButton}>←</button>
        ...
        <button onClick={login} className={styles.nextButton}>Next</button>
    </div>
);

const PersonalInfoPage = ({ signup, locationInfo }) => (
    <div className={styles.personalInfoPage}>
        <button onClick={signup} className={styles.backButton}>←</button>
        <h2>Tell us a little about yourself</h2>
        <label>
            Choose a unique username
            <input type="text" placeholder="teacher.s_pet123" className={styles.inputField} />
        </label>
        ...
        <p className={styles.infoText}>You can customize the visibility of your information in the settings</p>
        <button onClick={locationInfo} className={styles.nextButton}>Next</button>
    </div>
);

const LocationInfoPage = ({ personalInfo, tagsInfo }) => (
    <div className={styles.locationInfoPage}>
        <button onClick={personalInfo} className={styles.backButton}>←</button>
        ...
        <p className={styles.infoText}>You can customize the visibility of your information in the settings</p>
        <button onClick={tagsInfo} className={styles.nextButton}>Next</button>
    </div>
);

const TagsInfoPage = ({ locationInfo, interestsInfo }) => (
    <div className={styles.tagsInfoPage}>
        <button onClick={locationInfo} className={styles.backButton}>←</button>
        <h2>Where do you stand?</h2>
        <p>Choose the tags below that you think best describes yourself</p>
        <div className={styles.tagsButtons}>
            {["Left Liberal", "Right Rebel", "Middle Man", "Eco Warrior", "Tech Geek", "Fiscal Hawk", "Socialist Spark", "Diplomacy Devotee", "Freedom Fanatic", "Capitalist Crusader", "Green Guardian", "Rural Revolutionary", "Planet Preserver", "Urban Utopian", "Cultural Creative", "Justice Juggernaut", "Equality Evangelist"].map(tag => (
                <button key={tag} className={styles.tagButton}>{tag}</button>
            ))}
        </div>
        <button onClick={interestsInfo} className={styles.nextButton}>Next</button>
    </div>
);

const InterestsPage = ({ tagsInfo }) => (
    <div className={styles.interestsPage}>
        <button onClick={tagsInfo} className={styles.backButton}>←</button>
        ...
        <button onClick={tagsInfo} className={styles.finishButton}>Finish</button>
    </div>
);
...
const Onboarding = () => {
    ...
    const renderStep = () => {
        switch (currentStep) {
            ...
            case 2:
                return <SignUpPage onboarding={onBoarding} personalInfo={personalInfo} />;
            case 3:
                return <LoginPage onboarding={onBoarding} forgotPassword={forgotPassword} />;
            case 4:
                return <PersonalInfoPage signup={signUp} locationInfo={locationInfo} />;
            case 5:
                return <LocationInfoPage personalInfo={personalInfo} tagsInfo={tagsInfo} />;
            case 6:
                return <TagsInfoPage locationInfo={locationInfo} interestsInfo={interestsInfo} />;
            case 7:
                return <ForgotPasswordPage login={login} />;
            case 8:
                return <InterestsPage tagsInfo={tagsInfo} />;
            default:
                return <OnboardingPage signup={signUp} login={login} />;
        }
    };
